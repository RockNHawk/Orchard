...
namespace BinaryAnalysis.CultureFallback.Drivers {
    [UsedImplicitly]
    ...
    public class CultureBodyPartDriver : BodyPartDriver 
    {
        ...
        private string GetFallbackBodyText(BodyPart part) {
            ...
            if (string.IsNullOrWhiteSpace(partText) && siteCulture != currentCulture) {
                var localizationPart = part.ContentItem.Parts.OfType<LocalizationPart>().SingleOrDefault();
                if (localizationPart != null) {
                    var original = localizationPart.MasterContentItem.ContentItem.Parts.OfType<BodyPart>().SingleOrDefault();
                    return GetBodyText(original);
                }
            }
